...
#define DEVICE_NAMED                         "Prospeck_B"                                /**< Name of device. Will be included in the advertising data. */
...
#define MAX_HEART_RATE                       1500                                        /**< Maximum heart rate as returned by the simulated measurement function. */
...
#define MAX_RR_INTERVAL                      1500                                        /**< Maximum RR interval as returned by the simulated measurement function. */
...
void app_error_handler(uint32_t error_code, uint32_t line_num, const uint8_t * p_file_name)
{
	simple_uart_putstring("App_error_handler \n");
    // disable INTs
...
{
	simple_uart_putstring("assert_nrf_callback \n");
...
{
	//simple_uart_putstring("Battery update");
    uint32_t err_code;
...
{
	//simple_uart_putstring("battery timeout handler");
...
static uint16_t number_altering_test(uint16_t *input)
{
	double x = 60.0;
	double val = PI / 180;
	double ret = cos( x*val);
	unsigned char to_print[32];
	sprintf((char*)to_print, "X: %u\n", ret);
	(*input)++;
	//simple_uart_putstring(to_print);
	return *input;
}
...
{
	//simple_uart_putstring("heart rate timeout handler");
...
{
	//simple_uart_putstring("rr interval timeout handler");
    UNUSED_PARAMETER(p_context);
...
{
	//simple_uart_putstring("sensor contact timeout handler");
...
{
	simple_uart_putstring("advertising initialised \n");
    uint32_t      err_code;
...
{
	simple_uart_putstring("services initialised \n");
    uint32_t       err_code;
 ...
 *
 * @param[in]   p_evt   Event received from the Connect ion Parameters Module.
 */
...
static void on_ble_evt(ble_evt_t * p_ble_evt)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        case BLE_GAP_EVT_CONNECTED:
        	simple_uart_putstring("BLE Connected \n");
        ...
        case BLE_GAP_EVT_DISCONNECTED:
        	simple_uart_putstring("BLE Disconnected \n");
            nrf_gpio_pin_clear(CONNECTED_LED_PIN_NO);
        ...
        case BLE_GAP_EVT_TIMEOUT:
        	simple_uart_putstring("BLE Timeout \n");
        ...
        default:
        	simple_uart_putstring("BLE default \n");
...
static void ble_stack_init(void)
{
    ...
    APP_ERROR_CHECK(err_code);
    simple_uart_putstring("BLE stack initialised \n");
}
